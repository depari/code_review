...
Shopping GenerateShoppingListForTest() {
...
TEST(shopping_list_test, shopping_object_last_element_test) {
  Shopping shopping_object = GenerateShoppingListForTest();
  // Test 0 - Last element test
  EXPECT_EQ((shopping_object.GetLastElement()).GetPrice(), 30000);
  ASSERT_EQ((shopping_object.GetLastElement()).GetSerialNumber(), 2);


}

TEST(shopping_list_test, shopping_object_sortbyindex_test) {
  Shopping shopping_object = GenerateShoppingListForTest();
  ...
  ASSERT_EQ((shopping_object.GetLastElement()).GetSerialNumber(), 5);
}


TEST(shopping_list_test, shopping_object_sortbyprice_test) {
  Shopping shopping_object = GenerateShoppingListForTest();
  ...
  ASSERT_EQ((shopping_object.GetLastElement()).GetSerialNumber(), 4);
}
